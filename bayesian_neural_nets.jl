...
begin
	n = 2000
	x = [rand(n ÷ 2) * π .- 2π; rand(n ÷ 2) * π .+ π]
	y = sin.(x) .+ randn(length(x)) .* 0.225 .* abs.(sin.(1.5 .* x .+ π/8))
	x, y = gpu(x), gpu(y)
	nothing
end

# ╔═╡ 136001b1-3b58-4b56-aebd-66cd89e3e47d
begin
	ensamble_size = 3
	hidden_layers = 4
	hidden = 50
...
function logprobloss(m, x, y) 
    yhat = m(x)
    μ = yhat[1:1, :]
    logσ = softclip.(yhat[2:2, :], -3, 0)
    mean(((y .- μ) ./ exp.(logσ)) .^ 2 ./ 2 .+ logσ)
end
...
begin
	models = [
		Chain(
			Dense(1, hidden, activation_function), 
			[Dense(hidden, hidden, activation_function) for _ in 1:hidden_layers]..., 
			Dense(hidden, 2)
		) |> gpu
	...
	for (i, m) in enumerate(models)
		...
		ps = Flux.params(m)
		data = Flux.DataLoader((x', y'); batchsize, shuffle=true)
		opt = ADAM()
...
let xrange = -3π:0.01:3π,
	...
	logσs = [yo[2, :] for yo in yout]
	
	plt = plot(xrange, sin, label="true value")
	scatter!(plt, cpu(x), cpu(y), label="measured data")
	for i in eachindex(μs)
		plot!(plt, xrange, μs[i], ribbon=exp.(logσs[i]), fillalpha=0.5, label="model $i")
	end
	plt
end
...
let	xrange = -3π:0.01:3π,
	...
	logσs = mean([yo[2, :] for yo in yout])
	
	plot(xrange, sin, label="true value")
	scatter!(cpu(x), cpu(y), label="measured data")
	plot!(xrange, μs, ribbon=exp.(logσs), fillalpha=0.5, label="averaged model")
end
...
git-tree-sha1 = "1756e4846e13eecfce8e4db3b9d3a901041bded4"
