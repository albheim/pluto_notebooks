...
# ╔═╡ 1046fb88-90ab-11ec-1b9d-016e518a27bc
using Plots, PlutoUI, SignalAnalysis

# ╔═╡ eebc7408-2820-4719-bce6-f25423c3826c
md"ω = $(@bind ω Slider(0.1:0.1:5, show_value=true, default=1.0))"

# ╔═╡ 8de5dc94-3363-4e53-82cc-4d815afee1dc
md"ωs = $(@bind ωs Slider(0.1:0.1:10, show_value=true, default=1.0))"

# ╔═╡ 9dfb64b4-5b2f-4b84-b293-0824c68719ff
md"N = $(@bind N Slider(1:10, show_value=true, default=1))"

# ╔═╡ 37bf0e51-f91d-4e7d-b7ff-b04a01fe59b1
offset = pi

# ╔═╡ 2d035f0e-0dd9-4e93-9c0d-2d1e13326e3e
begin
	t = 0:0.001:10
	hs = 2pi/ωs
	ts = 0:hs:10
	ωa = abs(mod(ω + ωs / 2, ωs) - ωs / 2)
	y = sin.(ω .* t)
	ys = sin.(ω .* ts)
	ya = sin.(ωa .* t .+ offset)
	nothing
...
# ╔═╡ a384d5bd-8904-4e56-ae22-bff4e2397e76
begin 
	p = plot(t, y, label="Real data")
	scatter!(p, ts, ys, label="Sampled data")
	plot!(p, t, ya, label="Fundamental alias")
	p
end

# ╔═╡ 8cb2c1e6-5162-40e6-977b-5359cd5cfe6b
let	tt = 0:hs:10000,
	yy = sin.(ω .* tt)
...
# ╔═╡ e59bcda5-9c79-4575-8050-7216fca3ea6f
ys
