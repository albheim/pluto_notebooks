...
# ╔═╡ de9cb448-7315-11ec-22b8-4f6f9efd777c
using DifferentialEquations, Plots, PlutoUI

# ╔═╡ e07e4032-9406-42b1-97c1-4d3d29f9a50c
function f(du, u, p, t) # t is in days
	α, β, δ, N = p
	minscale = 0.1
	q = 2 * minscale / (1 - minscale)
	bt = β * (cos(2π * t / 365) + 1 + q) / (2 + q)
	du[1] = -bt * u[1] * u[2] / N - bt * u[1] * u[3] / N + α * u[4] + α * u[5] + α * u[8] # Suceptible
	du[2] = bt * u[1] * u[2] / N - δ * u[2] + α * u[7] # Infected by 1
	du[3] = bt * u[1] * u[3] / N - δ * u[3] + α * u[6] # Infected by 2
	du[4] = δ * u[2] - α * u[4] - bt * u[4] * u[3] / N # Recovered from 1, can still get 2
	du[5] = δ * u[3] - α * u[5] - bt * u[5] * u[2] / N # Recovered from 2, can still get 1
	du[6] = bt * u[4] * u[3] / N - δ * u[6] - α * u[6] # Recovered from 1, have 2
	du[7] = bt * u[5] * u[2] / N - δ * u[7] - α * u[7] # Recovered from 2, have 1
	du[8] = δ * u[6] + δ * u[7] - α * u[8] # Recovered from both
end

# ╔═╡ b26e20b6-d0ad-4a69-92f7-a8b20533f370
N = 10000

# ╔═╡ 210eb736-1182-402c-85a2-9b5905dd5991
tspan = (0, 2000.0)

# ╔═╡ e882697b-92ad-498b-8073-9d1b8cbff3bb
initial = [100, 1000]

# ╔═╡ 2b7a46c5-269a-4978-b349-70ad0741673a
u0 = [N - sum(initial); initial; zeros(5)]

# ╔═╡ a2fcd685-0939-427d-a5d7-6269eaa6d350
p = [1/300, 1, 0.2, N]

# ╔═╡ 5cae606e-fa92-4042-b425-da90e8be1c3c
prob = ODEProblem(f, u0, tspan, p)

# ╔═╡ 72fba9d7-11e0-4b14-8f4c-d5f87f9b395b
sol = solve(prob)

# ╔═╡ 4b9c31d6-3d3a-4a88-9764-6d32194bd6dc
plot(sol, xticks=0:365:tspan[2])
