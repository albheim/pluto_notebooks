...
# ╔═╡ 2092358e-8a8e-11ec-0ca3-f559fbd53845
using Flux, DifferentialEquations, Plots, PlutoUI, Statistics

# ╔═╡ 4da62970-9751-46c6-aaa2-37463ea1fd2c
...
# ╔═╡ 3651b564-2a4b-4dfa-a78f-c5a9329969a5
function gen_data(T, N)
	xdata = []
	ydata = []
	for i in 1:N
		u0 = randn(3)
		...
		sol = solve(prob)
		data = sol(0:0.01:T)
		append!(xdata, data[1:end-1])
		append!(ydata, data[2:end])
	end
	hcat(xdata...), hcat(ydata...)
end

# ╔═╡ ef371dbb-8b7d-4f63-9843-78baf958317c
x, y = gen_data(100, 100)

# ╔═╡ f6568dfc-4c5a-4eb2-bc08-cbbdfdc97897
xt, yt = gen_data(10, 10)

# ╔═╡ 769b4df9-9ee3-4687-81b1-f62f0ab498bf
begin
	traindata = Flux.DataLoader((x, y), batchsize=32, shuffle=true)
	
	m = Chain(
		Dense(3, 10, sigmoid),
		Dense(10, 10, x -> exp(-x^2)),
		Dense(10, 10),
		Dense(10, 3)
	)
	
	loss(x, y) = mean(abs.(m(x) .- y))
	opt = ADAM()
	ps = Flux.params(m)
	mycb() = @show loss(xt, yt)
	
	PlutoUI.with_terminal() do
		Flux.@epochs 10 Flux.train!(loss, ps, traindata, opt; cb=mycb)
	end
end
